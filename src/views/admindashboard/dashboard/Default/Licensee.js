...
const Licensee = ({ isLoading }) => {
  ...
  useEffect(() => {
    if (userDetails?.rows) {
      const licenseeData = userDetails.rows.filter((row) => row.role === 'licensee');
    ...
    <>
      {isLoading ? (
        ...
        <CardWrapper border={false} content={false}>
          <Box sx={{ p: 2 }}>
            {/* <List sx={{ py: 0 , textDecoration:'none'}} component={Link} to="/licensee">  */}

            <List
              sx={{ py: 0, textDecoration: 'none' }}
              component={!userDataDetailse.ftLogin ? Link : 'div'}
              to={!userDataDetailse.ftLogin ? '/licensee' : undefined}
            >
              <ListItem alignItems="center" disableGutters sx={{ py: 0 }}>
                ...
                <ListItemText
                  ...
                  primary={<Typography variant="h4">Licensee</Typography>}
                  secondary={
                    <Typography
                      variant="h5"
                      sx={{
