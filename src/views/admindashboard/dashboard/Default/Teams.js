...
const MyTeam = ({ isLoading }) => {
  ...
  useEffect(() => {
    if (userDetails?.rows) {
      const teamData = userDetails.rows.filter((row) => row.role !== 'licensee');
    ...
    <>
      {isLoading ? (
        ...
        <CardWrapper border={false} content={false}>
          <Box sx={{ p: 2 }}>
            {/* <List sx={{ py: 0 , textDecoration:'none'}} component={Link} to="/admin_AllTeam">  */}
            <List
              sx={{ py: 0, textDecoration: 'none' }}
              component={!userDataDetailse.ftLogin ? Link : 'div'}
              to={!userDataDetailse.ftLogin ? '/admin_AllTeam' : undefined}
            >
