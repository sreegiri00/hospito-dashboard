...
const MyTeam = ({ isLoading }) => {
  ...
  useEffect(() => {
    if (userDetails?.rows) {
      // Filter the user rows by specific roles
      const filteredUsers = userDetails.rows.filter(
        (row) => row.role === 'adminAcnt' || row.role === 'adminMngr' || row.role === 'adminOptr'
      );

      // Set the count of filtered users
    ...
    <>
      {isLoading ? (
        ...
        <CardWrapper border={false} content={false}>
          <Box sx={{ p: 2 }}>
            {/* <List sx={{ py: 0, textDecoration: 'none' }} component={Link} to="/licensee_myteam"> */}
            <List
              sx={{ py: 0, textDecoration: 'none' }}
              component={!userDataDetailse.ftLogin ? Link : 'div'}
              to={!userDataDetailse.ftLogin ? '/licensee_myteam' : undefined}
            >
